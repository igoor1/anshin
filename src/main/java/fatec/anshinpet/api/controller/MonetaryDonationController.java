...
public class MonetaryDonationController {
    ...
    public MonetaryDonationDTO create(@RequestBody MonetaryDonationInput donationInput) {
        return monetaryDonationService.create(donationInput);
    }

    @PutMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public MonetaryDonationDTO update(@PathVariable Long id, @RequestBody MonetaryDonationInput donationInput) {
